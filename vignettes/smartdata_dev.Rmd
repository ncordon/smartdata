...
`preprocess` is an `S3` method which assigns to an object `task` the name of the package the method is in as a class, and calls the function which resolves the task (which is going to be another `S3` method which would be different for each single package):
...
Inside each `{wrapper}.R` file, we will find, on the top part, a declaration of the available methods for that preprocessing with information about the package they come from (`pkg` slot), and the original name of the method in that package (`map` slot). For example, for `instanceSelection.R`:
...
We know from the information above that `CNN` method derives from the `unbalanced` package, and the `ubCNN` function inside it.

The names of the valid methods will be contained in a similar variable.

```{r wrapper-methods, eval = FALSE}
instSelectionMethods <- names(instSelectionPackages)
```

For each method, there should be a declared variable `arg.{method}` which would contain information for each of the accepted arguments for that method (`default` values in case those arguments omitted when calling the function -, a `check` function to ensure the parameter is passed correctly, and a string of information `info` about the parameter, which will be shown if `which_options` is called with the names of the wrapper and the method). Specifically, package `checkmate` has been used to provide verbose compact check functions (curryfied in the first argument with `Curry` method from `functional` package, i.e. the same as writing `function(x) { qexpect(x, rules = "foo")}`). As example:
...
If an argument is missing the `default` slot then a value must be provided for it when calling the function.

There should be an `S3` method which evaluates the wrapper for the methods included in a given package. As an example, let's observe the method that evaluates instance selections using methods from `unbalanced`:
...
A reason to use such an structure (resolve each call grouping methods per origin package instead of treating each method differently) is that methods taht come from the same package (usually) need similar adjustments (for example, order of columns).
...
* `checkDatasetClass`: checks that a certain class variable is present in a `data.frame`
* `mapArguments`: is used inside `doInstSelection.unbalanced` for example, and matches a list of arguments (present in `...` variable passed to the corresponding wrapper against `args.CNN` or `args.ENN` in the case of `unbalanced` package), maps its names to the original names the origin package needs and substitutes default arguments.
