...
Apart from the parameters for each specific method, an additional parameter `exclude` can be supplied with a vector of features names, so that those features are striped before the normalization and joined after the procedure, with the unchanged original data.
...
## Discretization
Discretizing a dataset consists in turning continuous variables into discrete attributes, that is, taking an attribute which can take real values and making a binning so that there exists a finite set of values for the variable. This procedure can have a great importance in procedures such as classification.
...
Similar to feature selection or normalization wrappers, `discretize` needs a `class_attr` in certain cases (e.g. methods like `equalfreq`, `equalwidth` or `globalequalwidth` do not need it, and will output an error if this argument is supplied to `discretize`).
...
Space transformation changes a training set $S = \{(x_i^{(1)}, \ldots x_i^{(n)})\}_{i=1^m}$ where $x_i^{(j)} \in \mathbb{R}$ into another set $S' = \{(\bar{x}_i^{(1)}, \ldots, \bar{x}_i^{(k)})\}_{i=1^m}$ with better properties for machine learning methods.
...
"multivariate" approach will imply deletion of the outlier instances from the dataset. "univariate" outliers can be treated imputing the median or the mean of the instances.
...
## Missing values
Missing values are tuples that lack from a certain attribute value, so a preliminary imputation could have a lot of benefits with respect to a posterior machine learning technique, such as classification.
...
## Noise
Noisy data are instances which include additional meaningless information appart from the true information they encode. Removing or repairing those instances prior to a classification, can have a potential benefit on it.
